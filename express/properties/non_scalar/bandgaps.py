...
class BandGaps(NonScalarProperty):
    ...
    def _serialize(self):
        return {
            'name': self.name,
            'values': [
                self.compute_on_mesh(type="direct"),
                self.compute_on_mesh(type="indirect")
            ]
        }

    def compute_on_mesh(self, type="indirect"):
        """
        ...
        if ns == 1:
            gap, k1, k2 = self._find_gap(N_sk[0], ev_sk[0], ec_sk[0], type=type)
        else:
            gap, k1, k2 = self._find_gap(N_sk.ravel(), ev_sk.ravel(), ec_sk.ravel(), type=type)
            k1 = divmod(k1, nk)
            k2 = divmod(k2, nk)
