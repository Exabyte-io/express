...
class EspressoTXTParser(BaseTXTParser):
    ...
    def phonon_frequencies(self, modes_file):
        ...
        with open(modes_file, 'r') as f:
            text = f.read()
        qpoints = np.array(re.compile(settings.REGEX["qpoints"]["regex"]).findall(text), dtype=np.float64)
        frequencies = np.array(re.compile(settings.REGEX["phonon_frequencies"]["regex"]).findall(text), dtype=np.float64)
