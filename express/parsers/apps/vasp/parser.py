...
from express.parsers.utils import find_file, find_files_by_name_substring
...
class VaspParser(BaseParser, IonicDataMixin, ElectronicDataMixin, ReciprocalDataMixin):
    ...
    def initial_structure_strings(self):
        structures = []
        for poscar in find_files_by_name_substring("POSCAR", self.work_dir):
    ...
    def final_structure_strings(self):
        structures = []
        for poscar in find_files_by_name_substring("CONTCAR", self.work_dir):
            with open(poscar) as f:
                structures.append(f.read())
        return structures
